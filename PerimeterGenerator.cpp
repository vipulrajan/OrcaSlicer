            ...
            if (extrusion->is_closed) {
                ...
                if (extrusion->inset_idx % 2 == 1) {
                    for (size_t path_idx = 0; path_idx < multi_path.paths.size(); path_idx++) {
                        ...
                        int total_layer_count = layer_count()
                        // first and last layers are printed regularly for print quality sake
                        if (perimeter_generator.layer_id == 1) // first layer needs more material to account for added height
							cur_path.extrusion_multiplier = 1.5;
                        else if (perimeter_generator.layer_id == total_layer_count-1) // last layer needs less material to account for removed height
                            cur_path._multipliextrusioner = 0.5;
                        else if (perimeter_generator.layer_id > 1 && perimeter_generator.layer_id < total_layer_count-1) // offset only applicable paths
