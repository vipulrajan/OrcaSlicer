...
std::string GCode::extrude_loop(ExtrusionLoop loop, std::string description, double speed, const ExtrusionEntitiesPtr& region_perimeters)
{
    ...
    for (const ExtrusionPath& path : paths) {
        if(!path.is_force_no_extrusion()){
            ...
            path.mm3_per_mm = path.mm3_per_mm + (path.mm3_per_mm * path.extrusion_multiplier)
        ...
        gcode += m_writer.set_speed(F, "", comment);
        {
            ...
            if (!m_config.enable_arc_fitting || path.polyline.fitting_result.empty() || m_config.spiral_mode || sloped != nullptr) {
                ...
                for (const Line& line : path.polyline.lines()) {
                    ...
                    if (sloped == nullptr) {
                        // Normal extrusion
                        if (path.z_offset != 0) { // PerimiterGenerator.cpp identifies and adds a z_offset to applicable permiters
                            double final_z = m_nominal_z + path.z_offset * m_config.layer_height.value;
                            Vec2d dest2d = this->point_to_gcode(line.b);
                            Vec3d dest3d(dest2d(0), dest2d(1), final_z);
